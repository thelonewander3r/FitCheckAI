    ...
    >
      ...
      <CardContent className="flex flex-col gap-4 flex-1">
        ...
        <div className="relative h-48 w-full rounded-lg overflow-hidden bg-[#f4f6f8] flex items-center justify-center">
          {vto?.renderedImageUrl && !imageError ? (
            ...
            <div className="flex flex-col items-center gap-2 text-[#718096]">
              ...
              <span className="text-xs">{vto ? "Preview unavailable" : "Try on to preview"}</span>
            </div>
          )}
        </div>

        {/* Description */}
        <p className="text-xs text-[#4a5568] leading-relaxed">{outfit.description}</p>

        {/* Garments */}
        <div>
          <p className="text-xs font-semibold text-[#0f2744] mb-1.5">Includes</p>
        ...
        <div className="space-y-2">
          ...
          <ScoreBar label="Format suitability" score={outfit.scores.interviewFormatSuitability} />
          <ScoreBar label="Budget fit" score={outfit.scores.budgetFit} />
          <ScoreBar label="Camera readiness" score={outfit.scores.cameraReadiness} />
        ...
        <div className="mt-auto pt-2">
          <p className="text-xs font-semibold text-[#0f2744]">
            Est.{" "}
            <span className="text-[#2a6f7f]">${outfit.estimatedPrice.toFixed(0)}</span>
          </p>
          {outfit.genderNeutralNote && (
            <p className="mt-1 text-[10px] text-[#718096] italic">{outfit.genderNeutralNote}</p>
          )}
        </div>
